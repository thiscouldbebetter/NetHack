...
main(int argc UNUSED, char *argv[] UNUSED)
{
    ...
    Fprintf(ofp, "/* glyph, ttychar, { %s%s } */\n",
            "glyphflags, { COLOR_CODE_NONE, symidx }, NO_CUSTOMCOLOR, NO_CUSTOMCOLOR, ovidx, tileidx", enhanced);
#ifdef ENHANCED_SYMBOLS
    enhanced = ", 0"; /* replace ", utf8rep" since we're done with that */
#endif
    Fprintf(ofp, "const glyph_info nul_glyphinfo = { \n");
    Fprintf(ofp, "%sNO_GLYPH, ' ', COLOR_CODE_NONE,\n", indent);
    Fprintf(ofp, "%s%s/* glyph_map */\n", indent, indent);
    Fprintf(ofp, "%s%s{ %s, TILE_UNEXPLORED%s }\n", indent, indent,
            "MG_UNEXPL, { COLOR_CODE_NONE, SYM_UNEXPLORED + SYM_OFF_X }, NO_CUSTOMCOLOR, NO_CUSTOMCOLOR",
            enhanced);
    Fprintf(ofp, "};\n");
    ...
    for (i = 0; i < MAX_GLYPH; i++) {
        ...
        Fprintf(ofp,
                "    { 0U, { COLOR_CODE_NONE, 0 }, NO_CUSTOMCOLOR, NO_CUSTOMCOLOR, %4d%s },   /* [%04d] %s:%03d %s */\n",
                tilenum, enhanced, i,
