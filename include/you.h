...
struct carrying_special_objects {
...
struct Race {
	...
	const char *adjective;      /* adjective ("human", "elven") */
	const char *collective;     /* collective ("humanity", "elvenkind") */
...
struct you {
	...
	dungeon_and_level_numbers uz, uz0;    /* your level on this and the previous turn */
	dungeon_and_level_numbers utolev;     /* level monster teleported you to, or uz */
	uchar utotype;      /* bitmask of goto_level() flags for utolev */
	dungeon_and_level_numbers ucamefrom;  /* level where you came from; used for tutorial */
...
#define LEFT_HANDED  0x01
	...
	unsigned turnsAsDemigodSoFar;           /* how long you have been demigod */
	struct event_history player_event_history;      /* certain events have happened */
	struct carrying_special_objects player_carrying_special_objects;
	struct u_conduct uconduct;  /* KMH, conduct */
	struct u_roleplay uroleplay;
	struct attribs attributes_current,              /* your current attributes (eg. str)*/
					attributes_from_exercise,       /* for gain/loss via "exercise" */
					attributes_bonus,               /* your bonus attributes (eg. str) */
					attributes_max,                 /* your max attributes (eg. str) */
					attributes_temporary,           /* used for temporary loss/gain */
					attributes_temporary_countdown; /* used for loss/gain countdown */
	align alignment;               /* character alignment */
#define CONVERT    2
#define A_ORIGINAL 1
#define A_CURRENT  0
...
#define LUCKMIN (-10) /* minimum value of u.uluck */
	schar hit_increment;
	schar damage_increment;
	schar armor_class;
#define AC_MAX    99  /* abs(u.uac) <= 99; likewise for monster AC */
	uchar spell_protection;                           /* protection by SPE_PROTECTION */
	uchar moves_until_spell_protection_degrades;      /* #moves until uspellprot-- */
	uchar moves_between_spell_protection_degradation; /* #moves between uspellprot-- */
	int hit_points, hit_points_max,      /* hit points, aka health */
		hit_points_peak;                   /* highest value of uhpmax so far */
	int energy, energy_max,              /* magical energy, aka spell power */
		energy_peak;                       /* highest value of uenmax so far */
	xint16 hit_points_increment[MAXULEV],  /* increases to uhpmax for each level gain */
		  energy_increment[MAXULEV];       /* increases to uenmax for each level gain */
	int have_angered_gods;                 /* if the gods are angry at you */
	int artifacts_bestowed_count;          /* number of artifacts bestowed */
	int blessed, blessing_duration;                 /* blessing/duration from #pray */
	long money0;
	long uspare1;
	long experience, urexp;          /* exper pts for gaining levels and for score */
	long cleansed_moves;             /* to record moves when player was cleansed */
	long sleeping_move_last_started; /* sleeping; monstermove you last started */
	int in_a_vault;
	struct monster *monster_stuck_to;     /* engulfer or grabber, maybe grabbee if Upolyd */
	struct monster *monster_being_ridden; /* mount when riding */
	unsigned monster_stuck_to_id;         /* u.ustuck->m_id, used during save/restore */
	unsigned monster_being_ridden_id;     /* u.usteed->m_id, used during save/restore */
	long turns_steed_runs_after_kick;     /* turns steed will run after being kicked */
	int turns_spent_riding;               /* time spent riding, for skill advancement */
	int times_died;          /* how many times you died */
	int risen_from_grave;    /* you die and become something aside from a ghost */
	int weapon_slots;        /* unused skill slots */
	int skills_advanced;     /* # of advances made so far */
	xint16 skill_record[P_SKILL_LIMIT]; /* skill advancements */
	struct skills weapon_skills[P_NUM_SKILLS];
	boolean using_two_weapons; /* KMH -- Using two-weapon combat */
	short mcham;               /* vampire mndx if shapeshifted to bat/cloud */
	short movement;            /* instead of youmonst.movement */
	schar achievements_in_order_obtained[N_ACH];    /* list of achievements in the order attained */
}; /* end of `struct you' */
...
struct _hitmon_data {
	...
	boolean silver_message;
	boolean silver_object;
	boolean light_object;
	int material;
	int jousting;
	boolean hit_text;
	boolean get_damage_bonus;
	boolean unarmed;
	boolean hand_to_hand;
	boolean ispoisoned;
	boolean unpoisonmsg;
	boolean need_poison_message;
	boolean poiskilled;
